...
export default function SampleDownloadPage() {
  ...
  useEffect(() => {
    const start = async () => {
      try {
        setStatus('preparing');
        const res = await fetch(sampleDownloadUrl);
        if (!res.ok) throw new Error('Download failed');
        const blob = await res.blob();
        const disposition = res.headers.get('content-disposition') || '';
        let filename = 'StudyCrate-sample.pdf';
      ...
      <div className="max-w-md w-full space-y-3 text-center">
        ...
        {status !== 'error' && (
          <p className="text-sm" style={{ color: 'var(--color-text-secondary)' }}>
            We are fetching your file from the server. This may take a few seconds on the first request.
          </p>
        )}
